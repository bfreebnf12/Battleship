...
// Function to check for overlapping ships when placing a new ship
const hasOverlapWithOtherShips = (coords) => {
    for (const ship of ships) {
...
const placeShip = (size) => {
    ...
    do {
        ...
        for (let i = 1; i < size; i++) {
            ...
            if (hasDuplicateCoord([newCoord]) || hasOverlapWithOtherShips([newCoord])) {
    ...
    } while (coord.length !== size || hasDuplicateCoord(coord) || hasOverlapWithOtherShips(coord));
...
const playGame = () => {
    resetShips(); // Reset the ship coordinates for each new game
    createShips();
    ...
    // Reset fleetSizeVariable to its initial value for a new game
    fleetSizeVariable = 5;
};
